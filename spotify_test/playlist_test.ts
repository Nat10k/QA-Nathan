...
Scenario('edit the playlist details', async ({ I }) => {
    ...
    I.fillField(locate('input').withAttr({type:'text', 'data-testid':'playlist-edit-details-name-input'}).inside(editModal), 'New Playlist');
    I.fillField(locate('textarea').withAttr({'data-testid':'playlist-edit-details-description-input'}).inside(editModal), 'New Playlist Description');
    I.click(locate('button').withAttr({'data-testid':'playlist-edit-details-save-button'}).inside(editModal));
    I.waitForText('New Playlist', 5);
});

Scenario('play the playlist', async ({ I }) => {
    I.click(locate('div').withAttr({role:'button'}).inside(locate('ul').withAttr({role:'list', 'aria-label':'Your Library'})));
    I.waitForText('New Playlist', 10);
...
Scenario('delete playlist', async ({ I }) => {
    ...
    I.click(locate('button').withChild(locate('span').withText('Delete')).inside(locate('div').withAttr({role:'dialog', 'aria-label':'Delete New Playlist?'})));
});
